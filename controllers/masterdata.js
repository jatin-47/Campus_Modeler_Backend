...
exports.campusBuildings = async (request, response, next) => {
	try {
		let CampusBuildings = await CampusBuilding.find(
			{ campusname: request.user.campusname },
			"BuildingID BuildingName BuildingType NoOfFloors NoOfWorkers Status"
		);
...
exports.viewDataCampusBuildings = async (request, response, next) => {
	try {
		...
		let viewdata = await CampusBuilding.findOne({
...
exports.uploadCampusBuildings = async (request, response, next) => {
	try {
		...
		try {
			...
			return next(
				new ErrorResponse(
					"Could not delete the temp file!(internal err) " + err,
					500
				)
			);
		}
	...
	try {
        ...
        */
		for (let BuildingName in data) {
			let building = await CampusBuilding.findOne({
			...
			for (let i = 1; i <= building.NoOfFloors; i++) {
				...
				let tobeModifiedRooms = building.Rooms.filter(
					(room) => room.Floor == i
				);
...
exports.addBuildingCampusBuildings = async (request, response, next) => {
	try {
		...
		if (building) {
			...
			// create a new building and return roomdetails
...
exports.deleteBuildingCampusBuildings = async (request, response, next) => {
	try {
		const { BuildingID } = request.query;
		await CampusBuilding.deleteOne({
			campusname: request.user.campusname,
			BuildingID: BuildingID,
		});
...
exports.classSchedule = async (request, response, next) => {
	try {
		let ClassSchedules = await ClassSchedule.find(
			{ campusname: request.user.campusname },
			"CourseID CourseName BuildingName RoomName Strength Departments Status"
		);
...
exports.viewDetailsClassSchedule = async (request, response, next) => {
	try {
		...
		let viewdata = await ClassSchedule.findOne({
			CourseID: CourseID,
			campusname: request.user.campusname,
		});
		response.send(viewdata);
	...
	for (let i = 0; i < courses.length; i++) {
		for (let j = 0; j < courses[i].CourseInstructor.length; j++) {
			let fac = await Faculty.findOne({
				Name: courses[i].CourseInstructor,
			}).exec();
			if (fac == null || fac == undefined) continue;
...
exports.addClassClassSchedule = async (request, response, next) => {
	try {
		...
		if (!building)
			throw "No such building found in which this class is scheduled!";

		const room = building.Rooms.filter(
			(room) => room.RoomName == data.RoomName.trim()
		);
		...
		let batch_id = data.StudentComposition.map(async (batch) => {
			let Batch = await BatchStudent.findOne({
...
exports.getBuildingAddClassClassSchedule = async (request, response, next) => {
	try {
		const buildingnames = await CampusBuilding.find(
			{ campusname: request.user.campusname },
			"BuildingName"
		);
...
exports.getRoomIdAddClassClassSchedule = async (request, response, next) => {
	try {
		const { BuildingId } = request.query;
		const building = await CampusBuilding.findOne(
			{ campusname: request.user.campusname, BuildingId: BuildingId },
			"Rooms"
		);
...
exports.getCourseInstructorAddClassClassSchedule = async (
	request,
	response,
	next
) => {
...
//Not used
exports.getStudentStrengthAddClassClassSchedule = async (
	request,
	response,
	next
) => {
...
exports.addStudentCompositionAddClassClassSchedule = async (
	request,
	response,
	next
) => {
...
exports.editStudentCompositionAddClassClassSchedule = async (
	request,
	response,
	next
) => {
...
exports.deleteStudentCompositionAddClassClassSchedule = async (
	request,
	response,
	next
) => {
...
exports.users = async (request, response, next) => {
	try {
		let viewdata = await User.find(
			{ campusname: request.user.campusname },
			"UserID username role email contact status"
		);
		response.send(viewdata);
	} catch (err) {
		return next(new ErrorResponse(err, 400));
...
exports.viewDetailsUsers = async (request, response, next) => {
	try {
		...
		let userdata = await User.findOne(
			{ _id: UserID, campusname: request.user.campusname },
			"+photo_path"
		);
		...
		for (let BatchName in data) {
			(async function () {
				for (let CourseName in data[BatchName]) {
					let course = await ClassSchedule.findOne({
...
exports.templatefacultydetails = async (request, response, next) => {
	...
	sheet.columns = [
		...
		{ header: "Department Building Name" },
		{ header: "No of Adult Family Members" },
...
exports.residenceBuildNameAddFacultyDetails = async (
	request,
	response,
	next
) => {
	try {
		let allResidenceBuildings = await CampusBuilding.find(
			{
				BuildingType: "Faculty Residence",
				campusname: request.user.campusname,
			},
			"BuildingName"
		);
