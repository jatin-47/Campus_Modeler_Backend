...
class VirusModel(TruthClassStatus):
    ...
    def __init__(self):
        ...
        self.testing_model = {1: self.random_testing, 2: self.symptom_testing, 3: self.contacttracing_testing,
                              4: self.risk_testing , 5:self.hostel_testing}
    ...
    def __daily_transmissions__(self, c_calibrated=1):
        ...
        for person in temp_AFreeP:
            ...
            #  and time of contact P_TR = 0.015 # TODO: Dummy value for now start = time.time()
            ct = 0
        ...
        if len(self.positiveon_result_day) != 0:
            for i in self.positiveon_result_day[-1]:
                ...
                for j in range(len(contacts)):
                    ...
                    if edge_weights[j] > edge_threshold and temp_person_obj.state == 'Not_tested':
    ...
    def hostel_testing(self):
        ...
        new_list = []
        for i in students:
            if self.ID_to_person[i].state == 'Not_Tested':
        ...
        if len(new_list) > test_cap-self.num_of_people_in_queue:
            ...
            self.num_of_people_in_queue = self.num_of_people_in_queue + test_cap-self.num_of_people_in_queue
        else :
            list_key = new_list
    ...
    def random_enqueuing_and_random_people_retest_without_posipeople(self):
        loop_ctr = 0
        test_cap, students_only = self.check_validation()
        while self.num_of_people_in_queue < test_cap and TestingState.Num_not_tested != 0:
